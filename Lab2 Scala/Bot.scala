    ...
    def forSlave(bot: MiniBot) {
        bot.inputOrElse("mood", "Lurking") match {
            ...
            case "Defensive" => reactAsDefensiveMissile(bot)
             case "Being a landmine" => explodeIfNeeded(bot)
            case s: String => bot.log("unknown mood: " + s)
        }
    }
    
     def spawnLandmine(bot: Bot) {
        val offset = XY(random.nextInt(3)-1, random.nextInt(3)-1)
        bot.spawn(offset, "mood" -> "Being a landmine")
        bot.say("Spawning landmine bot")
    }
     def explodeIfNeeded(bot: MiniBot) {
        val offsetToBadBeast = bot.view.offsetToNearest('b')
                
        offsetToBadBeast match {
            case None =>
                val offsetToEnemyMaster = bot.view.offsetToNearest('m')
                offsetToEnemyMaster match {
                    case None =>
                        val offsetToNearestEnemyMiniBot = bot.view.offsetToNearest('s')
                        
                        offsetToNearestEnemyMiniBot match {
                            case None =>
                            case Some(nearest: XY) =>
                                if (nearest.length <= 2) {
                                    bot.say("Bazinga")
                                    bot.explode(4)
                                }
    ...
    */
    def getNeighbours(position: XY, bot: Bot): Set[XY] = {
        ...
        if (cellAbsPos.x >= 0 && cellAbsPos.x < size &&
            ...
            // W - wall, M - master bot, S - slave, P - plant, B - beast
            if (cellValue != 'W' && cellValue != 'M' && cellValue != 'S' &&
            cellValue != 's' && cellValue != 'p') {

            if (!bot.isSlave && cellValue != 'm' && cellValue != 'b') {
